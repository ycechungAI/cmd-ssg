  ...
  it("Should sanitize malicious route URL in HTML menu template", () => {
    ...
    const maliciousOptions = {
      ...
      routeList: [
        ...
        { url: "j\x00avascript:alert(1)", name: "test7" },
        { url: "java\nscript:alert(1)", name: "test8" },
        { url: " javascript:alert(1)", name: "test9" },
    ...
    expect(html).not.toContain("href='j\x00avascript:alert(1)'");
    expect(html).not.toContain("href='java\nscript:alert(1)'");
    expect(html).not.toContain("href=' javascript:alert(1)'");

    // Check that it replaced them with about:blank
    expect(html.match(/href='about:blank'/g).length).toBe(9);
