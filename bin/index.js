...
if (options.version) {
  ...
  if(options.config){
    ...
    configFile.output ? outputFolderLocal = configFile.output: outputFolderLocal = "dist";
  }
  let files = [];
  ...
  if (testInput == true) {
    ...
    console.log(`  running >>>`);
    helper.convertToHtml(options.input, options.stylesheet, outputFolderLocal, isFile);
  } else {
    //no input given
    helper.displayError(true, errorCode1, 1);
  }
