...
## 2026-03-22 - [XSS Bypass via Control Characters in URL Schemes]
**Vulnerability:** The URL sanitizer `sanitizeUrl` in `generateHtmlTemplate.js` only trimmed leading/trailing whitespace before checking for malicious schemes like `javascript:`. Attackers could embed control characters or whitespaces inside the scheme name (e.g. `java\nscript:alert(1)` or `java\x00script:alert(1)`) to bypass the filter and still achieve XSS.
**Learning:** Browsers ignore control characters and whitespaces within URL schemes. Simple prefix matching on sanitized strings is ineffective if internal whitespace and control characters are not first removed.
**Prevention:** When enforcing allowlists or denylists on URL schemes, always aggressively strip control characters (`\x00-\x20`) and whitespace (`\s`) from the entire string before performing the check.
